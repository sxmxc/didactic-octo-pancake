...
- **Egg drawer = live pack UI.** The Eggs toggle now surfaces the real pack/inventory workflow (daily claim, Brood/Nursery purchases, opening packs, hatch odds) so designers can play through the same system that players will use instead of seeded debug eggs.
...
- **UI + signals.** `Eventbus.player_egg_inventory_updated` emits the snapshot dictionary, and `scenes/ui/drawers/egg_catalog_drawer.*` renders everything: pack cards (claim/purchase/open buttons wired straight into the Player helpers), a scrollable egg inventory list showing tier + source, and live hatch-odds copy generated from the tier/species metadata. That drawer fully replaces the old debug egg catalog in `scenes/ui/world_view_menu.tscn`.
...
- **Stations + AI flow.** `buildables/training/gym_pod.tscn` and `buildables/training/study_desk.tscn` extend `buildables/training/training_station.gd`, register through `autoload/data.gd`, and are auto-unlocked for the player inside `scenes/world.gd::_bootstrap_player_profile()` so the build drawer can place them. The BeeHave branch (`creature_ai/conditions/ShouldTrainCondition.gd` + `creature_ai/actions/UseTrainingStationAction.gd`, wired inside `scenes/creature/creature.tscn`) scans every live `TrainingStation`, walks idle creatures toward the closest stat gap, and calls `Creature.begin_training_session()` so `Eventbus.training_*` signals and HUD snapshots keep streaming while the station locks itself to the active creature until completion.
- **Behavior tree exit.** Once a creature hits the exported fatigue or stat-gain goals inside `creature_ai/actions/UseTrainingStationAction.gd`, the action cancels the current session, returns `SUCCESS`, and lets the `Sequence*` composite advance per the [BeeHave action leaf guidance](https://github.com/bitbrain/beehave/blob/godot-4.x/docs/manual/first_behavior_tree.md). This prevents creatures from chaining sessions forever and hands control back to hunger/sleep actions while the rest grace timer refills.
