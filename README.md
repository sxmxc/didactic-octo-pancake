...
- Buildables are painted as scene tiles via `assets/tiles/world_map.tres` (TileSetScenesCollectionSource id 1) and dropped onto `%WorldMapLayer` by `scenes/ui/drop_area.gd`; tile ids map to scenes as `1: buildables/nests/nest.tscn`, `2: buildables/food_containers/food_container.tscn`, `3: buildables/training/gym_pod.tscn`, `4: buildables/training/study_desk.tscn`. Add new buildables to that TileSet before wiring drag targets.
...
## Diet system & food bag

- **Species diets.** `resources/creature/species.gd` now exports a `diet` enum (`meat`, `veggie`, `omnivore`), and every resource under `resources/creature/data/**` declares its diet so behavior, UI, and future evolution hooks can reason about feeding needs without guesswork.
- **Food definitions in data.** `autoload/data.gd` ships a `food_definitions` dictionary with three starter meals (Prime Cut, Leafy Bundle, Balanced Bento), each referencing `scenes/food/food.gd` plus tuning knobs for hunger relief, stat buffs/debuffs, and optional tint overrides so designers can expand the pantry without touching gameplay code.
- **Context-aware consumption.** `scenes/food/food.gd` enforces diet compatibility, applies hunger relief, adjusts stats via `Creature.apply_food_effects()` inside `scenes/creature/creature.gd`, and posts flavor notifications. `creature_ai/actions/WalkToFoodAction.gd` ignores incompatible foods, while `scenes/world.gd` and `scenes/player/player.gd` refuse to place meals that nobody can eat and surface warnings through `Eventbus`.
- **Food bag UX.** `scenes/ui/drawers/food_bag_drawer.tscn`/`.gd` render the definitions inside an option list with diet compatibility, hunger relief, stat summaries, and live creature-match counts. Dropping a meal instantiates the configured scene, calls `Food.configure_from_definition()`, and routes it through the usual `Eventbus.feed_request` flow so bowls only receive meaningful food, keeping future food-bowl variants grounded in the same data.
...
- **Live action/mood readouts.** Every action script calls `Creature.set_behavior_state()`, which broadcasts a `creature_activity_changed` signal (see `autoload/eventbus.gd`). The focus drawer (`scenes/ui/drawers/creature_details_drawer.tscn`) now surfaces the mood label, current action, and a rotating “thought” quip so players can read what each creature is doing when zoomed in. In-world, `scenes/creature/creature.gd` hands emotions + thoughts off to `scenes/creature/expressions/expression_controller.gd`, which instantiates `scenes/creature/thoughts/thought_bubble.tscn` once each emotion bubble times out so the same Tiny5 RichText thought appears over the creature with the handheld-style pixel font.
...
- **Egg drawer = live pack UI.** The Eggs toggle now surfaces the real pack/inventory workflow (daily claim, Brood/Nursery purchases, opening packs, hatch odds via a popup) so designers can play through the same system that players will use instead of seeded debug eggs.

## Egg inventory & packs

- **Persistent inventory resource.** `resources/eggs/egg_inventory.gd` sits behind `Player` as the single source of truth for egg tokens, pack counts, and per-pack cooldown timestamps. `Player.get_egg_inventory_snapshot()` now serializes that dictionary so the save payload keeps unopened packs, token metadata (tier, source pack, award time), and claim timers intact.
- **Pack + tier definitions live in data.** `autoload/data.gd` defines three packs out of the gate—`daily_single` (“Daily Meadow Drop”, one Meadow roll every 20 hours), `brood_bundle` (three eggs: two Meadow, one Tidepool for 250 gold), and `nursery_crate` (five eggs: three Meadow, one Tidepool, one Phantom for 550 gold). Each entry’s `egg_rolls` array lists `{tier_id, count}` pairs so designers can mix tiers per pack without touching gameplay code. `egg_tier_definitions` supplies display names/descriptions for Meadow/Tidepool/Phantom tiers.
- **Species supply the weights.** `resources/creature/baby_species.gd` now exports `egg_tier: StringName` and `hatch_weight: int`, and every baby `.tres` was updated accordingly (`blop`/`sprit` share the Meadow tier, `squip` lives in Tidepool, `ghos` in Phantom). When `Player.hatch_egg_at()` fires it asks `Game.roll_species_for_tier()`—a weighted draw built on those properties—before handing the chosen `BabySpecies` to `scenes/world.gd`.
- **Player/game/world flow.** `Player` swapped its ad-hoc array for an `EggInventory` instance plus helpers like `grant_pack()`, `open_pack()`, `try_claim_pack()`, `purchase_pack()`, `hatch_egg_at()`, and `restore_egg_token()` (used if spawning fails). `Game.sync_egg_rewards()` now runs on new sessions *and* after load to auto-deliver the Daily Meadow Drop whenever its cooldown expires, while `Game.roll_species_for_tier()` centralizes RNG so future tuning only touches one place. `scenes/world.gd` boots the player with a Brood Bundle opener, hatches the first creature from an egg, gates “New Creature” requests behind token availability, and restores tokens automatically if nests are full.
- **UI + signals.** `Eventbus.player_egg_inventory_updated` emits the snapshot dictionary, and `scenes/ui/drawers/egg_catalog_drawer.*` renders everything: pack cards (claim/purchase/open buttons wired straight into the Player helpers) that now only surface title/count/actions/status with their detail text moved into tooltips plus a click-to-open detail window, a scrollable egg inventory list whose entries launch the same detail window (with hatch buttons) for full token metadata, and a hatch-odds popup generated from the tier/species metadata—all opened via `scenes/ui/popup_window_manager.gd` to keep popups consistent. Simple notices still route through `Eventbus.popup_requested` into `scenes/ui/notification_window.tscn`. That drawer fully replaces the old debug egg catalog in `scenes/ui/world_view_menu.tscn`.
...
- **Needs overrides.** Training now aborts automatically if hunger climbs past ~70% full, energy dips under ~35%, or fatigue hits 75% of the cap so creatures can peel off to eat, nap, or recover before resuming another session. Stations receive the same `training_session_cancelled` signal as manual interrupts.
...
- **Stations + AI flow.** `buildables/training/gym_pod.tscn` and `buildables/training/study_desk.tscn` extend `buildables/training/training_station.gd`, register through `autoload/data.gd`, and are auto-unlocked for the player inside `scenes/world.gd::_bootstrap_player_profile()` so the build drawer can place them. The BeeHave branch (`creature_ai/conditions/ShouldTrainCondition.gd`, `creature_ai/actions/ApproachTrainingStationAction.gd`, and `creature_ai/actions/TrainAtStationAction.gd`, wired inside `scenes/creature/creature.tscn`) first scans every live `TrainingStation`, walks idle creatures toward the closest stat gap, then hands off to the dedicated training monitor so `Creature.begin_training_session()` can stream `Eventbus.training_*` payloads while the station stays locked to that creature.
- **Behavior tree exit.** Once a creature hits the exported fatigue or stat-gain goals inside `creature_ai/actions/TrainAtStationAction.gd`, the action cancels the current session, returns `SUCCESS`, and lets the `Sequence*` composite advance per the [BeeHave action leaf guidance](https://github.com/bitbrain/beehave/blob/godot-4.x/docs/manual/first_behavior_tree.md). This prevents creatures from chaining sessions forever and hands control back to hunger/sleep actions while the rest grace timer refills.
...
### Trait system (`resources/traits/*.gd`, `resources/stats/creature_stats.gd`, `scenes/creature/Creature.gd`, `autoload/eventbus.gd`, `scenes/ui/drawers/creature_details_drawer.*`)

- **Data definition + persistence.** `resources/traits/trait_catalog.gd` enumerates positive and negative traits (Metabolic Marvel, Bottomless Pit, Power Napper, Wired, Focused Student, Daydreamer, Tidy Freak, Nest Shedder) with tiered modifiers, descriptions, and evolution targets, while `resources/traits/trait_state.gd` stores per-creature state (id, tier, alignment, timestamps). `CreatureStats.traits` exports an `Array[TraitState]`, and `_stats_to_dict()` / `_apply_stats_from_dict()` serialize the array so saves capture every learned or inherited trait.
- **Birth rolls + scripting hooks.** When `scenes/creature/Creature.gd` boots it now requests one positive and one negative trait via `TraitCatalog.roll_random_state()`, stamps the source (`birth_positive`/`birth_negative`), and can later respond to scripted events through `Creature.add_trait()`, `Creature.upgrade_trait()`, and `Creature.remove_trait()`. Each change fires the new `Eventbus.trait_added|trait_upgraded|trait_removed` signals so UI, audio, or quest logic can react.
- **Simulation modifiers.** Traits contribute multiplicative modifiers that feed directly into hunger ticks, energy drain/recovery, training XP gain, fatigue accrual, and even BeeHave actions (self-grooming + nest tidying now reference the dynamically adjusted `grooming_relief_per_action` / `nest_cleanliness_restore_per_action`). Creature bootstrap and save-load flows recalc modifiers immediately so world ticks always see the right numbers.
- **Player feedback.** Every trait gain, upgrade, or loss enqueues a toast via `Eventbus.notification_requested` and refreshes the focus drawer. `scenes/ui/drawers/creature_details_drawer.*` now include a multi-line Traits section that lists each trait, alignment, and description so players understand why their creature is hungrier, tidier, or more efficient at training.
...
- **Telemetry + stats:** `autoload/telemetry.gd` taps into the new pack/egg signals emitted by `scenes/player/player.gd` and `autoload/eventbus.gd`, then forwards them through `Talo.events`/`Talo.stats` per the [events](https://docs.trytalo.com/docs/godot/events) and [stats](https://docs.trytalo.com/docs/godot/stats) guides. We currently publish `egg_pack_purchased`, `egg_pack_claimed`, `egg_pack_opened`, `egg_hatched`, `creature_adopted`, and `currency_earned` events plus the `egg_packs_purchased|claimed|opened`, `eggs_hatched`, `creatures_adopted`, and currency earned/spent stats. Make sure those stat names exist in the Talo dashboard before shipping builds, otherwise the increment calls will no-op.
- **Device auto-identification:** `scenes/login_screen.gd` loads/creates a stable device identifier (`OS.get_unique_id()` fallback stored in `user://device_identity.cfg`) and, after `Talo.init_completed`, calls `Talo.players.identify("device", identifier)` so every runtime build has a player alias before telemetry, saves, or storefront flows run. This follows the [identifying a player](https://docs.trytalo.com/docs/godot/identifying) guidance without prompting for usernames/passwords, and gives us a bridge until OAuth/Google Play hooks land.
